...
namespace cdm
{
DefaultMaterial::DefaultMaterial(const VulkanDevice& vulkanDevice,
                                 PbrShadingModel& shadingModel,
                                 uint32_t instancePoolSize)
    : Material(shadingModel, instancePoolSize),
      device(&vulkanDevice),
    ...
    sdw::FragmentWriter& writer, FragmentShaderBuildDataBase* shaderBuildData)
{
	...
	buildData->ssbo =
	    std::make_unique<sdw::ArraySsboT<shader::DefaultMaterialData>>(
	...
	MaterialFragmentFunction res = writer.implementFunction<Float>(
	    "DefaultFragment",
	    [&writer, buildData](
	        const UInt& inMaterialInstanceIndex, Vec4 inOutAlbedo,
	        Vec2 inOutUv, Vec3 inOutWsPosition, Vec3 inOutWsNormal,
	        Vec3 inOutWsTangent, Float inOutMetalness, Float inOutRoughness) {
		    // Locale(material,
