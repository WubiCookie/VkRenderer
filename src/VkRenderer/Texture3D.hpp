...
class Texture3D final : public TextureInterface
{
...
public:
	Texture3D() = default;
	Texture3D(RenderWindow& renderWindow, uint32_t imageWidth,
	          uint32_t imageHeight, uint32_t imageDepth, VkFormat imageFormat,
	...
	Texture3D(Texture3D&& texture) = default;
	~Texture3D();

	Texture3D& operator=(const Texture3D&) = delete;
	Texture3D& operator=(Texture3D&& texture) = default;
