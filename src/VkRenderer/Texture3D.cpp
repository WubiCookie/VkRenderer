...
namespace cdm
{
Texture3D::Texture3D(RenderWindow& renderWindow, uint32_t imageWidth,
                     uint32_t imageHeight, uint32_t imageDepth,
...
void Texture3D::transitionLayoutImmediate(VkImageLayout oldLayout,
...
void Texture3D::generateMipmapsImmediate(VkImageLayout currentLayout)
{
	if (rw.get() == nullptr)
		return;
	if (m_mipLevels <= 1)
	...
	int32_t mipDepth = int32_t(m_depth);
	...
	for (uint32_t i = 1; i < m_mipLevels; i++)
	{
		...
		blit.srcOffsets[1] = { int32_t(m_width), int32_t(m_height),
			                   int32_t(m_depth) };
...
void Texture3D::uploadDataImmediate(const void* texels, size_t size,
...
Buffer Texture3D::downloadDataToBufferImmediate(VkImageLayout currentLayout)
{
	return downloadDataToBufferImmediate(currentLayout, currentLayout);
}

Buffer Texture3D::downloadDataToBufferImmediate(VkImageLayout initialLayout,
                                                VkImageLayout finalLayout)
{
