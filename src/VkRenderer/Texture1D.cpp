...
namespace cdm
{
Texture1D::Texture1D(RenderWindow& renderWindow, uint32_t imageWidth,
...
void Texture1D::transitionLayoutImmediate(VkImageLayout oldLayout,
...
void Texture1D::generateMipmapsImmediate(VkImageLayout currentLayout)
{
	if (rw.get() == nullptr)
		return;
	if (m_mipLevels <= 1)
	...
	for (uint32_t i = 1; i < m_mipLevels; i++)
	{
		...
		blit.srcOffsets[1] = { int32_t(m_width), 1, 1 };
		...
		blit.dstOffsets[1] = { mipWidth > 1 ? mipWidth / 2 : 1, 1, 1 };
...
void Texture1D::uploadDataImmediate(const void* texels, size_t size,
...
Buffer Texture1D::downloadDataToBufferImmediate(VkImageLayout currentLayout)
{
	return downloadDataToBufferImmediate(currentLayout, currentLayout);
}

Buffer Texture1D::downloadDataToBufferImmediate(VkImageLayout initialLayout,
                                                VkImageLayout finalLayout)
{
